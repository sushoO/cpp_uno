...
int main(){
    ...
    /*
    int players; //this is an integer value of how many players are playing, which is later used as an argument for dealing the correct number of hands.
    ...
    std::vector<std::vector<std::string> > player_hand = sample.dealHand(); //Creates a vector of strings from the returned value of dealHand(), which generated different vectors for each player.
    ...
    sample.printHands();
    std::vector<std::vector <std::string> > handTest = sample.getHands();
